...
**General Availability cut.** First tag that meets the published GA
quality bar (see [OPERATIONS.md](./OPERATIONS.md)).

> **Why `0.1.1` and not `0.1.0`?** A pre-GA preview was published as
> `v0.1.0` on 2026-04-28. The GA quality bar (operational SLOs,
> runtime API-key rotation contract, cosign supply-chain story) was
> finalised after that preview cut. Rather than rewrite public git
> history, the first tag that meets the GA bar is `v0.1.1`. From
> this point forward chart and image versions move in lockstep with
> the operator's semver.

### Added

- **`agent.surface = "operator"` stamped on every batch.** The
  surface field carries fidelity-contract identity ("operator")
  separately from `agent.type` (which carries implementation identity
  — `k8s_operator`). The change is purely additive and
  backwards-compatible with v2.x of the wire format. New Go API:
  `wireformat.AgentSurface` enum + `wireformat.Agent.Surface` field.
  New helper: `cmd/main.go::newOperatorAgentFactory`.
- **Helm chart `keyRotation.workflow` value.** Documents the operator's
  recommended key-rotation workflow (`secret-on-reconcile` by default,
  `helm-upgrade` as a legacy fallback). Purely informational at the
  chart level — runtime behavior is unchanged.
- **Helm chart `networkPolicy` template (off by default).** Renders a
  NetworkPolicy that restricts the operator pod's egress to DNS, the
  Kubernetes API server, and configurable Incidentary backend
  hostnames. Optional ingress for Prometheus scrape via
  `networkPolicy.allowPrometheusScrape`.
- **Helm chart `serviceMonitor` template (off by default).** Renders a
  Prometheus Operator ServiceMonitor pointing at the operator's
  `:8080/metrics` endpoint. Requires `monitoring.coreos.com/v1` CRDs in
  the cluster.
- **`OPERATIONS.md` published SLOs.** Memory ceiling ≤ 512 MiB,
  CPU steady-state ≤ 0.2 cores, CPU peak ≤ 1 core, backend-unreachable
  resilience 24 h with < 30 s recovery and < 1% loss, leader-handoff
  loss < 1 event, helm upgrade zero-downtime on `replicaCount: 2`.
- **`SECURITY.md`.** Threat model, RBAC posture, supply-chain story
  (cosign keyless via GitHub OIDC), 14-day CVE response policy.
- **`ROADMAP.md`.** v0.1, v0.2 (mTLS for backend ingest), v0.3
  (incident-domain mapper integration).
- **Soak harness (`test/soak/run.sh` + `make soak`).** 7-day soak that
  measures the memory + CPU SLOs and writes a slo-summary.txt.
- **Backend-unreachable chaos harness (`test/chaos/backend-unreachable.sh`
  + `make chaos-backend-unreachable`).** 24-h NetworkPolicy block + recovery
  test for the resilience SLO.
- **Leader-rotation chaos harness (`test/chaos/leader-rotation.sh` +
  `make chaos-leader-rotation`).** 2-h leader-pod-kill loop for the
  leader-handoff SLO.
- **Helm upgrade zero-downtime test in CI (`test/e2e/helm-upgrade-e2e.sh`).**
  Polls `readyReplicas` every 200 ms during a rolling upgrade; fails
  if the count drops below 1.
- **Helm chart polish tests (`test/helm/{key-rotation-no-restart,chart-polish,release-workflow-cve-gate}.sh`).**
  Pure-template assertions wired into `make test-helm-templates` and
  the CI workflow.
- **Trivy CVE gate in release workflow.** Fails the release on
  HIGH/CRITICAL CVEs in fixed packages, before the chart push.

### Changed

- `DefaultAgentVersion` → `"0.1.1"`.
- README rewritten: replaced "alpha" header with explicit GA matrix.
- Chart and appVersion → `0.1.1`.
