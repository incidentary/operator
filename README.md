...
| Surface | State | Notes |
|---|---|---|
| Wire format | **stable** | v2 of [the Incidentary wire format](https://github.com/incidentary/wire-format/blob/main/spec/wire-format-v2.md). The operator stamps `agent.surface = "operator"` on every batch. |
| CRD shape | **stable for v0.1.1** | `IncidentaryConfig` v1alpha1; future minor versions remain backwards-compatible per Kubernetes API conventions. |
| Helm chart | **stable for v0.1.1** | `apiKey` + `workspaceId` are the only required values. NetworkPolicy + ServiceMonitor templates available, off by default. |
| Container image | multi-arch | `linux/amd64` + `linux/arm64`, cosign-signed at release time. |
| Kubernetes compatibility | **1.27+** | exercised continuously in CI on kind 0.23 (Kubernetes 1.30). |
| Operational SLOs | **published** | see [OPERATIONS.md](./OPERATIONS.md) — memory ≤ 512 MiB, CPU steady-state ≤ 0.2 cores, backend-unreachable resilience 24 h, leader-handoff loss < 1 event. |
| API-key rotation | **runtime, no pod restart** | Secret-on-reconcile; see [docs/operator/key-rotation](https://incidentary.com/docs/operator/key-rotation). |

Roadmap: [ROADMAP.md](./ROADMAP.md). Security posture: [SECURITY.md](./SECURITY.md).
...
```bash
helm install incidentary oci://ghcr.io/incidentary/operator/charts/incidentary-operator \
...
`v0.1.1` is the first GA release. `v0.1.0` was the preview tag. The
following changes have landed since the first preview cut and may
affect anyone running an older checkout. See [CHANGELOG.md](./CHANGELOG.md)
for the full list.
