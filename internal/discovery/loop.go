...
type Loop struct {
	...
	log         logr.Logger
	clusterName string
	// immutable cluster identifier sourced
	// from the `kube-system` Namespace UID at operator startup.
	// Empty when the lookup failed (legacy fallback to cluster_name).
	clusterUID     string
...
type Options struct {
	Interval    time.Duration
	ClusterName string
	// caller passes the kube-system
	// Namespace UID (or any other immutable cluster-unique value)
	// here. Empty values are tolerated; the topology report will
	// then carry only `cluster_name` and the backend keys on that.
	ClusterUID        string
...
func (l *Loop) runOnce(ctx context.Context) error {
	// resolve the kube-system Namespace UID
	// lazily on the first run if the caller didn't supply a value.
	// We do not fail the cycle on resolution error; the report just
	// goes out without `cluster_uid` and the backend falls back to
	// the legacy `cluster_name` keying.
	if l.clusterUID == "" {
		if uid := l.resolveClusterUID(ctx); uid != "" {
			l.clusterUID = uid
			l.log.Info("resolved cluster UID from kube-system namespace", "cluster_uid", uid)
		}
	}
...
func (l *Loop) deploymentToWorkload(ctx context.Context, d *appsv1.Deployment) ingestclient.TopologyWorkload {
	...
	return ingestclient.TopologyWorkload{
		...
		Labels:            d.Labels,
		Annotations:       filterSafeAnnotations(d.Annotations),
...
func (l *Loop) statefulSetToWorkload(ctx context.Context, s *appsv1.StatefulSet) ingestclient.TopologyWorkload {
	...
	return ingestclient.TopologyWorkload{
		...
		Labels:            s.Labels,
		Annotations:       filterSafeAnnotations(s.Annotations),
...
// resolveClusterUID fetches the `kube-system` Namespace and returns
// its UID, which is unique per cluster and stable across operator
// restarts. Returns "" on any failure (RBAC denial, namespace
// missing, transient API error). The empty string is propagated to
// the topology report's `omitempty` field so the backend falls back
// to the legacy cluster_name path.
//
func (l *Loop) resolveClusterUID(ctx context.Context) string {
	var ns corev1.Namespace
	if err := l.client.Get(ctx, client.ObjectKey{Name: "kube-system"}, &ns); err != nil {
		l.log.V(1).Info("could not resolve kube-system Namespace UID", "err", err.Error())
		return ""
	}
	return string(ns.UID)
}

// safeAnnotationPrefixes is the allowlist of annotation key prefixes
// the operator forwards to Incidentary verbatim. Annotations are a
// common storage location for credentials (Vault tokens, deploy
// tokens, embedded last-applied-configuration secrets); without an
// allowlist the operator would replicate them on every 5-minute
// topology cycle.
//
var safeAnnotationPrefixes = []string{
	"incidentary.com/",
	"app.kubernetes.io/",
...
// filterSafeAnnotations returns a copy of `in` that contains only
// allowlisted entries. Returns nil when no allowlisted entries exist
// so the JSON encoder emits the field as null rather than {}.
func filterSafeAnnotations(in map[string]string) map[string]string {
	if len(in) == 0 {
		return nil
	}
	out := make(map[string]string)
	for key, value := range in {
		for _, prefix := range safeAnnotationPrefixes {
