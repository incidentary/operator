...
func (r *Reconciler) classify(
	...
	//
	// these messages fire every reconciliation
	// cycle (~5 min by default) for every mismatched workload. Per-workload
	// namespace/name combinations may carry organisational-structure PII
	// or trade-secret-adjacent information that should not leak into
	// third-party log aggregators by default. Emit at V(1) so the
	// default log verbosity in production deployments suppresses them;
	// operators debugging classification can opt in by raising the
	// verbosity flag.
	if suggestion, ok := fuzzyMatch(w.ServiceID, registeredIDs, FuzzyMatchThreshold); ok {
		r.log.V(1).Info("workload service_id does not match any registered service",
