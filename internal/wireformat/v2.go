...
// AgentSurface identifies the *fidelity-contract* identity of the ingest
// surface — distinct from AgentType (implementation identity). Per D8 of
//
//
// The wire format keeps `agent.surface` optional in v2.x and required in
// v3.0; the operator always stamps AgentSurfaceOperator on every batch
// (the GA programme).
type AgentSurface string

const (
	AgentSurfaceExporter  AgentSurface = "exporter"
	AgentSurfaceProcessor AgentSurface = "processor"
	AgentSurfaceSDK       AgentSurface = "sdk"
	AgentSurfaceOperator  AgentSurface = "operator"
)
...
// AgentTypeK8sOperator and Surface to AgentSurfaceOperator.
//
// Surface is intentionally separate from Type (D8 of the plan): Type
// carries implementation identity ("k8s_operator" — what kind of
// software is sending), Surface carries fidelity-contract identity
// ("operator" — what the customer-visible surface is). They map
// 1:1 today but are decoupled at the wire so the server can stamp
// fidelity claims independently of agent identity.
//
// `omitempty` on Surface keeps the wire payload backwards-compatible
// for any caller that constructs an Agent without setting Surface
// (the v2.x spec lets the server derive a default from Type).
type Agent struct {
	Type        AgentType       `json:"type"`
	Version     string          `json:"version"`
	WorkspaceID string          `json:"workspace_id"`
	Surface     AgentSurface    `json:"surface,omitempty"`
...
type Event struct {
	...
	// Used for deduplication on retry. The operator emits UUIDv4 via
	// the ids.NewID() helper; the wire format accepts v1/v4/v7
	// transparently (identical 128-bit binary representation).
	ID string `json:"id"`
