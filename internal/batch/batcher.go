...
const (
	...
	// hard ceiling on the in-memory buffer
	// to prevent unbounded growth during persistent flush failures.
	// 100 × AbsoluteMaxBatchSize gives us headroom for ~1 M events
	// (roughly 100 MB at typical event sizes) before the operator
	// starts shedding load. Beyond this we drop new events rather
	// than OOM-kill the pod, which would lose ALL buffered events
	// and trigger a restart loop.
	BufferDropThreshold = AbsoluteMaxBatchSize * 100
)
...
type Batcher struct {
	...
	// X-Capture-Mode-Requested response header.
...
//
// when persistent flush failure (e.g. a
// network partition) causes the buffer to climb past
// BufferDropThreshold, this function tail-drops new events rather
// than allowing unbounded growth. Without the ceiling, a 15-minute
// API outage on a busy cluster grows the buffer past the operator's
// memory limit and OOM-kills the pod — losing ALL buffered events
// AND triggering a Lease churn that compounds the disruption.
func (b *Batcher) Enqueue(events ...wireformat.Event) {
	if len(events) == 0 {
		return
	}
	b.mu.Lock()
	current := len(b.buffer)
	if current >= BufferDropThreshold {
		b.mu.Unlock()
		// All incoming events are shed; counter increments by the
		// drop count so the dashboard can attribute the loss to
		// "buffer_full" rather than ingest rejection.
		metrics.EventsDroppedTotal.WithLabelValues("buffer_full").Add(float64(len(events)))
		return
	}
	// Partial admit: take as much as we can up to the threshold.
	available := BufferDropThreshold - current
	admitted := events
	if len(events) > available {
		admitted = events[:available]
		dropped := len(events) - available
		metrics.EventsDroppedTotal.WithLabelValues("buffer_full").Add(float64(dropped))
	}
	b.buffer = append(b.buffer, admitted...)
	size := len(b.buffer)
	b.mu.Unlock()
...
// header X-Capture-Mode-Requested (empty when never set). Future work wires this
