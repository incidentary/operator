...
// OnKeyRevoked installs dropping stubs immediately, marking the
// operator's outbound clients as disabled until the controller's
// next reconciliation delivers a fresh API key.
//
// without this, an HTTP 401 returned by
// the ingest backend (because the customer rotated/revoked the key
// for an incident response) would still leave the in-flight client
// holding the stale token. Subsequent flushes within the
// reconciliation interval would all fail with 401 and the lost
// batches would only be visible as scattered Error logs. By
// installing dropping stubs the moment a 401 is observed we (a)
// stop attempting writes with the bad key, (b) flip metrics to the
// disabled state so dashboards alert, and (c) preserve the
// workspaceID for downstream wiring. The controller's next
// reconciliation rotates back to a real client when a valid key
// reappears.
func (p *Provider) OnKeyRevoked(log logr.Logger) {
	p.mu.Lock()
	p.ingest = &droppingIngestClient{log: log}
	p.topo = &droppingTopologyClient{log: log}
	p.svc = emptyServicesClient{}
	p.mu.Unlock()
	log.Error(nil, "API key revoked (HTTP 401); dropping outbound clients until controller delivers a fresh key")
}
