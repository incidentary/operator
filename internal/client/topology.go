...
//
// `cluster_name` is a free-form string and
// is therefore unsafe as the sole cross-cluster discriminator. Two
// clusters under the same workspace that share a `cluster_name`
// (or both default to `"unknown"` when the operator's
// `K8S_CLUSTER_NAME` env var is empty) merge their workload lists
// on the backend, polluting one cluster's ghost-service list with
// the other's workloads. The new `cluster_uid` field carries an
// immutable identifier (the `kube-system` Namespace UID, which is
// unique per cluster and stable across operator restarts) so the
// backend can key on `(workspace_id, cluster_uid)` instead of
// `(workspace_id, cluster_name)`. The field is additive — older
// backends that ignore it fall back to the existing name-keyed path.
