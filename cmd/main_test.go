...
// ----------------------------------------------------------------------------
// production agent factory stamps Surface = "operator"
//
// Plan: (D1 + D8).
// The runtime agent factory used by the Batcher MUST produce an Agent
// with Surface = AgentSurfaceOperator on every call. This is the
// production code path — not the test-pipeline closure that builds a
// minimal Agent.
// ----------------------------------------------------------------------------

type stubWorkspaceProvider struct{ ws string }

func (s *stubWorkspaceProvider) WorkspaceID() string { return s.ws }

func TestNewOperatorAgentFactory_StampsOperatorSurface(t *testing.T) {
	t.Parallel()

	provider := &stubWorkspaceProvider{ws: "org_test_phase1"}
	factory := newOperatorAgentFactory(provider, "0.1.0-test")

	got := factory()

	if got.Type != wireformat.AgentTypeK8sOperator {
		t.Errorf("agent.type = %q, want %q", got.Type, wireformat.AgentTypeK8sOperator)
	}
	if got.Surface != wireformat.AgentSurfaceOperator {
		t.Errorf("agent.surface = %q, want %q (per plan D1/D8/)",
			got.Surface, wireformat.AgentSurfaceOperator)
	}
	if got.Version != "0.1.0-test" {
		t.Errorf("agent.version = %q, want %q", got.Version, "0.1.0-test")
	}
	if got.WorkspaceID != "org_test_phase1" {
		t.Errorf("agent.workspace_id = %q, want %q", got.WorkspaceID, "org_test_phase1")
	}
}

func TestNewOperatorAgentFactory_PicksUpRotatedWorkspaceID(t *testing.T) {
	t.Parallel()

	// The factory closure resolves WorkspaceID() on every call — the
	// rotation path of D5 (Secret-on-reconcile) depends on this. If the
	// factory captured WorkspaceID at construction time, a rotated
	// workspace ID would never reach the wire.
	provider := &stubWorkspaceProvider{ws: "org_initial"}
	factory := newOperatorAgentFactory(provider, "0.1.0-test")

	if got := factory(); got.WorkspaceID != "org_initial" {
		t.Fatalf("first call: agent.workspace_id = %q, want %q", got.WorkspaceID, "org_initial")
	}

	provider.ws = "org_rotated"

	if got := factory(); got.WorkspaceID != "org_rotated" {
		t.Errorf("after rotation: agent.workspace_id = %q, want %q",
			got.WorkspaceID, "org_rotated")
	}
}
