...
func main() {
	...
	// pin LeaseDuration / RenewDeadline /
	// RetryPeriod explicitly so the failover characteristics are
	// part of the deployment contract rather than whatever
	// controller-runtime defaults happen to ship. Defaults below
	// match controller-runtime's recommended ratios but are now
	// auditable. The values trade off failover latency (longer
	// = slower failover after pod death) against false-positive
	// re-elections (shorter = more leases lost during transient
	// apiserver hiccups). 30/20/4 gives ~30s worst-case downtime
	// during a clean rolling upgrade, with 4-second retry granularity.
	...
	agent := newOperatorAgentFactory(provider, ingestclient.DefaultAgentVersion)
...
// workspaceIDProvider is the minimal contract `newOperatorAgentFactory`
// needs from `*client.Provider`. Defined here so the unit test in
// `cmd/main_test.go` can satisfy it with a stub instead of pulling in
// a full `*client.Provider`.
type workspaceIDProvider interface {
	WorkspaceID() string
}

// newOperatorAgentFactory returns an agent-factory closure that the
// Batcher invokes on every flush. Every Agent it produces carries:
//
//   - Type    = wireformat.AgentTypeK8sOperator (implementation identity)
//   - Surface = wireformat.AgentSurfaceOperator (fidelity-contract identity, D8)
//   - Version = the supplied agent version
//   - WorkspaceID = the provider's current workspace ID at flush time
//
// of
func newOperatorAgentFactory(provider workspaceIDProvider, version string) func() wireformat.Agent {
	return func() wireformat.Agent {
